...
**Modules are isolated** — system modules run in-process via importlib. User modules run as subprocesses and communicate exclusively through the WebSocket Module Bus (`ws://core/api/v1/bus`). Direct access to core data and the `/secure` partition is prohibited.
...
| **User modules** (`type=UI/INTEGRATION/DRIVER/AUTOMATION/IMPORT_SOURCE`) | Subprocesses in Docker containers | Per-container overhead | WebSocket Module Bus at `ws://core/api/v1/bus?token=TOKEN` | Installed via marketplace |
...
**Endpoint:** `ws://core/api/v1/bus?token=TOKEN`
...
smarthome dev                         # start mock Core API + bus on :80
...
```bash
smarthome dev
# Starts a mock server on localhost
# Includes WebSocket Module Bus at ws://localhost/api/v1/bus
...
- Swagger UI: `http://smarthome.local:80/docs` (auto-generated by FastAPI)
...
- [ ] Core API :80 is not accessible from outside localhost (test via external IP)

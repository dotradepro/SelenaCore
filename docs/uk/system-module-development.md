...
- **Маршрутизація інтентів**: `get_module_for_intent("media.play")` повертає `"media-player"`
- **Розв'язання сутностей**: `get_modules_for_entity("radio")` повертає `["media-player"]`
- **Розрізнення пристроїв**: коли інтент цільовий тип сутності має кілька пристроїв, система запитує користувача уточнити
...
| `backup_manager` | Локальне резервне копіювання (ручне + за розкладом); хмарне + QR — фаза 2 |
| `remote_access` | Інтеграція з Tailscale VPN |
| `network_scanner` | Виявлення мережевих пристроїв (ARP, mDNS, SSDP) |
| `device_control` | Менеджер розумних пристроїв (Tuya через tuya-device-sharing-sdk) + інтенти `device.on/off` |

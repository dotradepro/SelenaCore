...
Мікрофон (arecord, ALSA)
  ...
  Модуль виконує дію через EventBus
     |
     v
  Native Piper TTS (HTTP, порт 5100)
     '-- Основний голос (мова системи, GPU, preloaded)
     |
     v
...
| `tts_lang` | Конфіг Piper `voice.tts.primary.lang` | Мова відповіді |

Правила:
- Усі відповіді озвучує **єдиний основний голос** з `voice.tts.primary.voice`.
  Мульти-мовні фрази (наприклад українська з латинськими словами як "WiFi")
  озвучуються основним голосом без розбиття.
...
Одна модель PiperVoice завантажується при старті на **нативному**
сервісі хоста (`piper-tts.service`) і тримається гарячою в GPU. У
контейнері пакету `piper-tts` немає — синтез делегується по HTTP на
`localhost:5100`.

| Голос | Призначення | Модель | GPU | RAM |
|-------|-------------|--------|-----|-----|
| Основний | Мова системи | uk_UA-ukrainian_tts-medium | Так | ~700-800 МБ (з onnxruntime-gpu) |

Мульти-мовні фрази (наприклад "Вмикаю WiFi") озвучуються основним
голосом цілком. Латинські слова можуть звучати незвично — це
компроміс задля економії ~150 МБ GPU-пам'яті та простішого пайплайну.

...
### Динамічний LLM промпт (тільки англійською)

З v0.4 уся LLM-класифікація інтентів працює **англійською**. Базовий
системний промпт читається з `intent_system` у prompt store (завжди з
`lang="en"` незалежно від мови TTS) і містить ~200 токенів. Динамічний
каталог дописується під час виклику:

- Зареєстровані інтенти згруповані по namespace (`device.on|off|...`)
- Активні модулі з їх списками інтентів
- Пристрої по кімнатах (з `meta.location_en` та `meta.name_en` —
  тільки англійські форми)
- Радіостанції / сцени (англійські назви)

Каталог будується свіжо при кожному виклику. Жодних мовних інструкцій
не додається — вивід завжди англійський. Мова користувача обробляється
[системою перекладу](translation.md) на краях пайплайну.

### Таби в Голос та AI

| Таб | Призначення |
|-----|-------------|
| **Engines** | Вибір / встановлення Vosk STT, Piper TTS, Ollama LLM |
| **Переклад** | Встановлення / активація мовних пар Argos Translate |
| **Промпти** | Перегляд активних системних промптів (5 ключів) |
| **Інтенти** | Усі зареєстровані голосові інтенти від системних та user-модулів |
| **Патерни** | Скомпільовані regex-патерни з FastMatcher |
| **Test** | Симуляція голосових команд через текст — повний трейс |

Таб **Промпти** є **тільки для перегляду**. Промпти керуються системою
внутрішньо і копіюються однаково на всі мовні рядки в DB
(`en`/`uk`/`ru`/...) — більше немає перекладу промптів за мовою,
оскільки ядро працює англійською.

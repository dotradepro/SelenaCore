...
> **Статус Phase 5/6:** Тільки V2. Дашборд рендериться з `widgetLayout.positions` для фіксованої сітки 5×4; room-таби фільтрують за `manifest.room` модуля. Зміни layout розповсюджуються через ту саму подію `layout_changed`, описану нижче. Pre-V2 поля layout більше не емітяться.

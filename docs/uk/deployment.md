...
Локальне резервне копіювання реалізує системний модуль `backup_manager` —
повний опис див. у [backup.md](backup.md).

Коротко:

- **Ручне резервування**: відкрийте сторінку *Налаштування* модуля на
  дашборді й натисніть *Створити зараз*. Архіви потрапляють до
  `/var/lib/selena/backups/`.
- **За розкладом**: на тій самій сторінці налаштувань оберіть *Щодня*,
  *Щотижня* або власний cron-вираз. Заплановані запуски виконує системний
  модуль `scheduler`.
- **Відновлення**: натисніть *Відновити* поряд з архівом у списку та введіть
  ім'я архіву для підтвердження. Перед відновленням автоматично створюється
  знімок поточного стану, після успішного відновлення `selena-core`
  перезапускається через `systemctl`.
- **Категорії**: категорія *core* (БД SQLite, розкладка дашборду, стан
  модулів, `/etc/selena`) включена завжди; категорія *secrets* (`/secure/`
  без `vault_key`) — опційно.

Для офлайн-копії без модуля скопіюйте директорії даних та секретів вручну:

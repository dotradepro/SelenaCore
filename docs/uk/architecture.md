...
7. Scan system_modules/ -> load in-process -> mount routers
   |  24 вбудовані модулі активовано
   v
...
**Джерело:** `src/components/dashboard/`, `core/api/routes/module_data.py`, `core/api/routes/scenes.py`

Phase 5/6 рекрафт дашборду — точкою композиції є `DashboardV2.tsx`. Замінює iframe-only V1-сітку Phase 4:

- **Hero-панель** — привітання, годинник, status pill, ярлики сцен.
- **Room-таби** — фільтрація віджетів за `manifest.room` (`system`, `home`, власні).
- **Фіксована сітка 5×4** — `BentoGrid.tsx` з явним `widgetLayout.positions`. Drag-drop у edit-режимі, порожні слоти стають drop-таргетами.
- **Template engine** — 8 вбудованих рендерерів (`metric`, `sparkline`, `toggle-list`, `control-panel`, `status`, `weather`, `media`, `presence`). Модулі оголошують `ui.widget.kind: "template"` і віддають JSON-payload з `data_endpoints[k]`. Iframe-віджети (`kind: "custom"`) залишаються підтримуваним fallback-варіантом.
- **Proxy-шар** — дашборд тягне дані віджетів через `/api/v1/modules/{module}/data/{key}` і викликає дії через `/api/v1/modules/{module}/action/{key}`. Core кешує за `cache_ttl_s` та інвалідує на події з `ui.widget.refresh.events`.
- **Активація сцен** — `POST /api/v1/scenes/{id}/activate` генерує `scene.activate` / `scene.activated` / `scene.failed`.

> Повна специфікація Phase 5/6, payload-схеми кожного шаблону та UX edit-режиму — у [dashboard-recraft.md](dashboard-recraft.md).
...
Маршрутизатор інтентів використовує 5-рівневий каскад. Кожен рівень перевіряється по порядку; перший збіг виграє. Уся pipeline працює **англійською внутрішньо** — починаючи з v0.4 переклад виконується [Argos Translate](translation.md) на краях пайплайну (після Vosk STT, перед Piper TTS), а не LLM. Українських / російських / німецьких FastMatcher-патернів немає за дизайном.
...
Повні деталі імплементації — pattern specificity scoring, composite resolver, дисамбігуація ambiguous імен, структура промпту, межі масштабування — у [intent-routing.md](intent-routing.md).

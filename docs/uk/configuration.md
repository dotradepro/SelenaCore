...
| Змінна | Тип | За замовчуванням | Опис |
|--------|-----|-----------------|------|
| `CORE_PORT` | `int` | `7070` | TCP-порт, на якому слухає API-сервер core. |
...
| Змінна | Тип | За замовчуванням | Опис |
|--------|-----|-----------------|------|
| `UI_PORT` | `int` | `80` | TCP-порт, на якому обслуговується веб-інтерфейс. |
| `UI_HTTPS` | `bool` | `True` | Чи обслуговувати UI через HTTPS. |
...
| `port` | `int` | `7070` | TCP-порт API-сервера core. |
...
| Ключ | Тип | За замовчуванням | Опис |
|------|-----|-----------------|------|
| `check_interval` | `int` | `30` | Інтервал у секундах між перевірками стану модулів. |
...
| `APP_URL` | Базова URL-адреса API core (наприклад, `http://localhost:7070`). Використовується зовнішніми сервісами, яким потрібно робити зворотні виклики до SelenaCore. |

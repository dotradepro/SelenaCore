...
> **Як писати наміри, що справді розпізнаються** — поле `description` порівнюється через cosine голосовим класифікатором, тож його формулювання напряму визначає точність. Перед додаванням нового наміра прочитайте [intent-authoring.md](intent-authoring.md) — рецепт description, правила якорів, особливості Helsinki UK→EN та PR-гейт через bench. Ігнорування зазвичай дає новому наміру точність 50-70% замість ≥ 90%.

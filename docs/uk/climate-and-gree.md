...
>
> **➡ Також див.:** [provider-system-and-modules.md](provider-system-and-modules.md) — пост-Gree рефакторинг, який перетворив device-control на runtime-плаговану систему провайдерів, додав SYSTEM-модуль `lights-switches`, об'єднав налаштування energy-monitor в одну фільтровану таблицю та переробив віджет climate під компактні плитки дашборду. Драйвер Gree і модуль Climate, описані нижче, залишаються канонічною реалізацією; цей документ описує їхній початковий дизайн.

...
def detect_audio_devices() -> AudioDevices:
    ...
    # PulseAudio / PipeWire — Bluetooth
    if _is_pulse_running():
        for sink in _pactl_list_sinks():
            if "bluez" in sink["name"]:
                devices.outputs.append(
                    AudioDevice(id=sink["name"], name=sink["description"], type="bluetooth")
                )
        for source in _pactl_list_sources():
            if "bluez" in source["name"]:
                devices.inputs.append(
                    AudioDevice(id=source["name"], name=source["description"], type="bluetooth")
                )
